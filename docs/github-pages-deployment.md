...
- 正式知识库名称：`COOC 课程内容管理`。
- 管理员负责成员、目录和公开边界；课程负责人使用“可编辑的成员”，审核人员使用“可阅读的成员”。
- 知识库保持“仅当前知识库成员可见”，公众入口始终是 GitHub Pages。
- 发布器只读取 `公开课程` 的直接 `docx` 子文档；`内部资料` 及其他目录不会发布。
- 成员授权、编辑、审核和撤回步骤见 [飞书课程维护与 GitHub Pages 发布 HOW-TO](feishu-course-maintenance-how-to.md)。

## 同步后部署

飞书同步通过 `GITHUB_TOKEN` 写入生成的公开站点；这类提交不会触发另一个推送工作流。因此 `sync-feishu.yml` 在生成和测试成功后检出最新 `main` 并直接部署 `site/` 到 Pages。不要改为依赖推送触发。

工作流可手动运行，也会在每天北京时间 10:17 自动执行。手动发布完成后，应检查工作流为绿色 `Success`，再匿名访问公开首页和本次修改的课程页。
